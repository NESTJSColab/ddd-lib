...
import { DomainGuard, getDomainEventHandler } from './utils';
...
> extends DomainEntity<TAggregateProps> {
  ...
  public setVersion(value: number): void {
    if (!DomainGuard.isNumber(value))
      throw new DomainException('The version number has to be a number');

    if (!DomainGuard.isNumberBetween(value, 0, 9999))
      throw new DomainException('The version number is not valid');
